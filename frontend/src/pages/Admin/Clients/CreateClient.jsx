import { faBirthdayCake, faBuilding, faCity, faFlag, faHouseUser, faIdBadge, faIdCard, faMailBulk, faPhone, faRoad, faSortNumericUpAlt, faStreetView } from "@fortawesome/free-solid-svg-icons";
...
const CreateClient = ({ buttonRef, refreshClients }) => {
    ...
    const alertMissingField = useCallback(
        (field_name) => {
            createAlert(`Ocorreu um erro ao criar o cliente, o campo ${field_name} é invalido`, "danger", true);
        },
        [createAlert]
    );
    ...
    const registerNewClient = useCallback(async () => {
        ...
        const data = await authenticatedFetch("company/client/create", {
            body: {
                cpf_cnpj,
                first_name,
                last_name,
        ...
        if (!data.data && !data.success) {
            createAlert("Ocorreu um erro ao criar o cliente, por favor contate um administrador", "danger", true);
            return;
        }

        if (data.data && data.data.code && data.data.code === 2) {
            createAlert("Ocorreu um erro ao criar o cliente, você já possui um funcionaro com esse CPF", "danger", true);
            return;
        }

        if (data.data && data.data.code && data.data.code === 1) {
            createAlert("Ocorreu um erro ao criar o cliente, você já possui um cliente com esse CPF", "danger", true);
            return;
        }

        createAlert("Cliente registrado com sucesso!", "success", true);
        form.reset();
        refreshClients();
    }, [authenticatedFetch, createAlert, alertMissingField, refreshClients]);
    return (
        <>
            {alert}
            <FullScreenModal title={"Registrar novo cliente"} toggleButtonRef={buttonRef}>
                <FullScreenModalBody>
                    ...
                    <p>A senha temporaria do cliente será enviada para o e-mail cadastrado, onde o mesmo poderá realizar as devidas alterações.</p>
                </FullScreenModalBody>
