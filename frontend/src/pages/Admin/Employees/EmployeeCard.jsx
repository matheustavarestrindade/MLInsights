...
import { ClientCardStyles } from "./EmployeeStyles";

const EmployeeCard = ({ client, refreshEmployees }) => {
    ...
    const [employee_fullname, setEmployeeFullName] = useState();

    const deleteClient = useCallback(async () => {
        const cpf_cnpj = client.cpf_cnpj;
        const data = await authenticatedFetch("company/employee/delete", {
            body: {
                cpf_cnpj,
            },
        });

        if (data.data && data.data.code === 2) {
            createAlert("Ocorreu um erro ao deletar o funcionario, não existe um funcionaro cadastrado com esse CPF", "danger", false);
            return;
        }

        if (data.data.success) {
            createAlert("Funcionario deletado com sucesso!", "success", false);
            refreshEmployees();
        }
    }, [authenticatedFetch, createAlert, client.cpf_cnpj, refreshEmployees]);
    ...
    const alertMissingField = useCallback(
        (field_name) => {
            createAlert(`Ocorreu um erro ao criar o funcionario, o campo ${field_name} é invalido`, "danger", false);
        },
        [createAlert]
    );
    ...
    const updateClientInfo = useCallback(async () => {
        ...
        const data = await authenticatedFetch("company/employee/update", {
            body: {
                cpf_cnpj,
                first_name,
                last_name,
        ...
        if (!data.data && !data.success) {
            createAlert("Ocorreu um erro ao atualizar o funcionario, por favor contate um administrador", "danger", false);
            return;
        }

        if (data.data && data.data.code && data.data.code === 3) {
            createAlert("Ocorreu um erro ao atualizar o funcionario, não foi possivel achar um funcionario com esse CPF", "danger", false);
            return;
        }
        if (data.data && data.data.code && data.data.code === 2) {
            createAlert("Ocorreu um erro ao atualizar o funcionario, você já possui um funcionaro com esse CPF", "danger", false);
            return;
        }

        createAlert("funcionario atualizado com sucesso!", "success", false);
        setHasChangedInfo(false);
        refreshEmployees();
    }, [hasChangedInfo, createAlert, authenticatedFetch, alertMissingField, refreshEmployees]);

    useEffect(() => {
        if (!client) {
            return;
        }
        setEmployeeFullName(capitalizeFirstLetter(client.first_name) + " " + capitalizeFirstLetter(client.last_name));
    ...
    return (
        <>
            ...
            <li className="list-group-item p-0">
                <FullScreenModal title={"Descartar alterações do funcionario"} toggleButtonRef={discardChangesButtonRef}>
                    <FullScreenModalBody>
                        <p>Você desejá mesmo descartar alterações feitas no funcionario {employee_fullname}?</p>
                    </FullScreenModalBody>
                ...
                <FullScreenModal title={"Resetar Senha do funcionario"} toggleButtonRef={resetPasswordButtonRef}>
                    <FullScreenModalBody>
                        <p>Você desejá mesmo resetar a senha de {employee_fullname}?</p>
                    </FullScreenModalBody>
                    <FullScreenModalFooter>
                ...
                <FullScreenModal title={"Deletar funcionario"} toggleButtonRef={deleteButtonRef}>
                    <FullScreenModalBody>
                        <p>Você desejá mesmo deletar o funcionario {employee_fullname}?</p>
                    </FullScreenModalBody>
                    <FullScreenModalFooter>
                ...
                <ClientCardStyles className="card card-body py-0">
                    <div className="row">
                        <div className="col-12 d-flex align-items-center justify-content-between py-3 collapse-button" ref={collapseRef}>
                            <div className="d-flex align-items-center justify-content-center">
                                <div className="client-icon me-3">
                                    <FontAwesomeIcon icon={faUser} />
                                </div>
                                <span className="m-0 fw-bold text-center">{employee_fullname}</span>
                            </div>
                        </div>
                            ...
                            <CollapseField buttonRef={collapseRef}>
                                ...
                                <div className="row mb-3">
                                    ...
                                    <div className="col-12 d-flex align-items-center justify-content-between border-bottom py-2">
                                        <span>Editar horarios de atendimento</span>
                                        <Link className="btn btn-primary text-white me-2" to={`/employees/calendar/${client.id}`}>
